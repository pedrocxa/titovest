...
export default function App() {
  ...
  const mKey = `_${selectedMonth}`; // Chave dinâmica para isolar dados por mês
  ...
  // --- Estados Persistentes (Reativos ao Mês Selecionado) ---
  ...
  const [investmentsBr, setInvestmentsBr] = useLocalStorage('titovest_inv_br' + mKey, []);
  ...
  // --- SEGURANÇA CONTRA CRASH (WHITE SCREEN) ---
  // Garante que o sistema usa Arrays válidos, mesmo que haja lixo antigo no LocalStorage
  ...
  const invBrList = Array.isArray(investmentsBr) ? investmentsBr : [];
  ...
  const [newExtInv, setNewExtInv] = useState({ type: 'Dólar (USD)', name: '', amount: '' });
  ...
  useEffect(() => {
    ...
    setGoalIndex(0); // Reseta o carrossel ao mudar de mês
  }, [salary, selectedMonth]);

  // --- Matemática e Inteligência Financeira ---
  ...
  // Lógica de Cores de Alerta (Negativos)
  ...
  // Investimentos Reais 
  const totalInvestmentsBr = invBrList.reduce((acc, inv) => acc + inv.amount, 0);
  ...
  const handleDeleteTx = (id) => {
    setTransactions(tList.filter(t => t.id !== id));
  ...
  const removeFixedCost = (id) => {
    setFixedCosts(fcList.filter(c => c.id !== id));
  ...
  const handleAddInvestment = () => {
    ...
    setInvestmentsBr([...invBrList, nInv]);
    setInvModal(false);
  ...
  const handleAddExtInvestment = () => {
    ...
    const nInv = {
      id: Date.now(),
      type: newExtInv.type,
      name: newExtInv.name || newExtInv.type,
      amount: parseFloat(newExtInv.amount)
    };
    setInvestmentsExt([...invExtList, nInv]);
    setExtInvModal(false);
    setNewExtInv({ type: 'Dólar (USD)', name: '', amount: '' });
    showToast('Investimento adicionado!');
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6">
          {fcList.length === 0 ? (
             <div className="sm:col-span-2 lg:col-span-4 flex items-center justify-center p-6 bg-white border border-dashed border-gray-200 rounded-2xl text-gray-400 text-sm text-center">
               Adicione dados na aba Carteira.
             </div>
          ) : (
            fcList.slice(0, 4).map((cost) => {
          ...
          <div onClick={() => setActiveTab('wallet')} className="clean-card p-5 md:p-6 flex flex-row sm:flex-col items-center sm:justify-center border-dashed border-2 border-gray-100 bg-gray-50/20 hover:border-red-100 hover:bg-red-50/10 cursor-pointer group transition-colors gap-4 sm:gap-0">
            ...
            <div className="flex flex-col flex-1 sm:items-center">
               <span className="text-sm font-medium text-gray-700 sm:mb-1 sm:text-center">Gerir custos fixos</span>
                ...
                {gList.map((goal, index) => {
                    ...
                    return (
                      <div key={goal.id} className="min-w-full flex flex-col md:flex-row items-center gap-6 md:gap-12 px-2 md:px-10 group relative">
                        {/* Botão de Excluir Meta no Dashboard */}
                        <button onClick={() => {
                  ...
                  <div key={inv.id} className="flex flex-col p-3 md:p-4 bg-white border border-gray-100 shadow-sm rounded-xl hover:border-emerald-100 transition-colors group">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <div className="flex items-center gap-3">
                         ...
                         <button onClick={() => {
                           setInvestmentsBr(invBrList.filter(c => c.id !== inv.id));
                           showToast('Renda fixa removida');
                         }} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-4 h-4"/></button>
                      </div>
                    </div>
  ...
  return (
    <>
      ...
      <div className={`flex h-screen w-full overflow-hidden relative transition-colors duration-300 ${isDarkMode ? 'theme-dark bg-[#0a0a0a]' : 'bg-[#f8f9fa]'}`}>
                ...
                <div className="relative">
                  ...
                  {showNotifs && (
                    <div className="absolute right-0 mt-3 w-64 md:w-72 bg-white border border-gray-100 rounded-2xl shadow-xl z-50 py-2 animate-fade-in transition-colors duration-300">
                      <div className="px-4 md:px-5 py-4 hover:bg-gray-50 cursor-pointer transition-colors border-gray-50 text-center text-sm text-gray-500">
        ...
        {/* MODAL: Novo Investimento Externo/Personalizado */}
        {extInvModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 bg-gray-900/20 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-[24px] w-full max-w-sm shadow-2xl border border-gray-100 overflow-hidden mx-auto transition-colors duration-300">
              <div className="flex justify-between items-center p-6 md:p-8 border-b border-gray-50 transition-colors duration-300">
              ...
              <div className="p-6 md:p-8 space-y-4 md:space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] md:text-xs font-medium text-gray-500 uppercase tracking-widest ml-1">Valor Convertido (R$)</label>
